...
<html>
  <head>
    ...
    <title>我的晒单_<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%></title>
  	<script language="javascript" type="text/javascript" src="/js/postsingleEdit.js?data=20131121"></script>
            ...
            <div class="ui-form ui-border-t">
                ...
                <ul class="upload">
                    <div class="ui-form-item ui-form-item-textarea ui-border-b">
                        ...
                        <textarea style=" font-size:12px; color:#706f6f" cols="30" rows="1" class="title" name="title" value="" id="postTitle">${shareinfo.shareTitle}</textarea>
                    </div>
                    <div class="ui-form-item ui-form-item-textarea ui-border-b">
                        <label style=" font-size:16px; color:#706f6f">感&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;言:</label>
                        <textarea style=" font-size:12px; color:#706f6f" cols="30" rows="10" class="textarea" name="ads_code" id="postContent">${shareinfo.shareContent}</textarea>
                    </div>
                    <li><span class="pic"></span>
                         <div class="uppic">
                             ...
                             <br/><br/>
                             <c:choose>
                             	<c:when test="${empty shareimageList}">
                             		<input type="hidden" name="hidPicAll" value="" id="hidPicAll" />
                             		<div style="display: none;" id="ulPicBox" class="pic"></div>
                             	</c:when>
                             	<c:otherwise>
                             		<input type="hidden" name="hidPicAll" value="${hidPicUrl}" id="hidPicAll" />
                             		<div id="ulPicBox" class="pic">
	                             	 <c:forEach items="${shareimageList}" var="shareImage">
	                             	 	<div id="img${fn:substring(shareImage.images,0, fn:indexOf(shareImage.images,'.'))}"><img src="/UserPost/Small/${shareImage.images}" alt=""><a href="javascript:;" title="删除此图片" onclick="javascript:delPostPic('${shareImage.images}');" />删除此图片</a></div>
	                             	 </c:forEach>
                             		</div>
                             	</c:otherwise>
                             </c:choose>
