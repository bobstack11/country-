		    ...
		    <ul id="ulPayway">
		    	<c:choose>
		    		<c:when test="${(user.commissionPoints/100)>=1}">
		    			<li class="gray6 z-pay-ff"><i id="spPoints" class="z-pay-ment" sel="0"></i><span>积分支付<em class="orange"><fmt:formatNumber value="${0}" type="number" maxFractionDigits="0" pattern="0" />.00</em>元（您的积分：${user.commissionPoints }）</span></li>
		    		</c:when>
		    		<c:otherwise>
		    			<li class="gray6 z-pay-ff z-pay-grayC"><i id="spPoints" class="z-pay-mentDis" sel="0"></i><span>您的积分不足（您的积分：${user.commissionPoints }）</span></li>
		    		</c:otherwise>
		    	</c:choose>
			    <c:choose>
