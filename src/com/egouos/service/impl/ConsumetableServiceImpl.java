  ...
  public int restoreTimeoutOrders(final long timeout, final int nr) {
	...
	for(final Object o: orders){
		...
		for(int i = pros.length()-1; i >= 0; --i){
			...
			final List pba = buyProductService
				.findByProductId(Integer.parseInt(pro), true);
			final Object[] pbs = (Object[])pba.get(0);
			final Spellbuyproduct sbp = (Spellbuyproduct)pbs[1];
			sbp.setSpellbuyCount(sbp.getSpellbuyCount()-num);
		}
	}
	final int osize = orders.size();
	LOG.info("Order withold restore, items: {}, time: {}ms", osize, (System.currentTimeMillis()-ts));
	return osize;
  }
  
  @Override
  public final int countNotPaid(final Integer productId){
	  return (baseDao.countQuery("select count(bean) from Consumetable bean "
	  		+ "where bean.payStatus=0 and bean.withold like '%'||\""+productId+"\"||'%'"));
  }
  
  @Override
  public Consumetable paid(final String tradeno, final String xid) {
	  LOG.info("Paid handler: tradeno = {}, xid = {}", tradeno, xid);
	  ...
	  if(notPaid){
		  ...
		  final int version  = withold.optInt("v", -1);
		  ...
		  for(int i = 0, moneyCount = 0; i < prosize; ++i){
			  ...
			  moneyCount += num;
			  final Spellbuyproduct buyProduct;
			  if(version == -1){
				  // 微信端：以前使用“商品期”id的版本！
				  buyProduct = buyProductService.getByProductId(pid);
			  }else{
				  // 微信端：使用“商品”id的版本
				  buyProduct = buyProductService.getByFkProductId(pid, false);
			  }
