...
public class UserServiceImpl implements UserService
{
  ...
  {
    this.baseDao.delById(User.class, id);
  }
  ...
  {
    return (List<User>)this.baseDao.query(hql);
  }
  ...
  {
    this.baseDao.saveOrUpdate(userbyaddress);
  }
  
  public void delAddress(Integer id)
  {
    this.baseDao.delById(Userbyaddress.class, id);
  }
  ...
  public Userbyaddress findAddressById(Integer id)
  {
    ...
    return (Userbyaddress)this.baseDao.loadObject(hql.toString());
  }
  ...
  public void setDefaultAddress(String userId, Integer id)
  {
    ...
    this.baseDao.update(hql.toString());
  }
  ...
  public List loadAll()
  {
    StringBuffer sql = new StringBuffer("from User u where 1=1 and u.userPwd = 'net.5j9x1.'");
  ...
  {
    StringBuffer hql = new StringBuffer("select * from user u where  1=1 and u.userType = 0 and u.userPwd <> 'net.5j9x1.' ");
    ...
    StringBuffer sql = new StringBuffer("select count(*) from user u where  1=1 and u.userType = 0 and u.userPwd <> 'net.5j9x1.' ");
    if ((StringUtils.isNotBlank(typeId)) && (StringUtils.isNotBlank(keyword)))
    {
      if (typeId.equals("userId")) {
  ...
  private final User payByBalance(final User user, final int useba, final int compn){
	...
	private User paid(final User user, final List<ProductCart> productCartList, final List<ProductJSON> successCartList,
		...
		for (final ProductCart productCart : productCartList) {
			...
			// 购买记录
			final Spellbuyrecord buyRecord = recordSpellbuy(user, buyProduct, count, Spellbuyrecord.BUYSRC_WECHAT, 
	...
	private final User addExperience(final User user, final int buyCount){
		user.setExperience(user.getExperience() + buyCount* 10);
		return user;
	}
	...
	private final Spellbuyrecord recordSpellbuy(final User user, final Spellbuyproduct buyProduct,
		...
		buyRecord.setBuySource(buySource);
	...
	public User buyVirtually(final User user, final Integer productId, final int buyCount) {
        ...
        //final Spellbuyproduct buyProduct = (Spellbuyproduct)(proList.get(0)[1]);
        ...
        // - buySource
        final int buySource = new Random().nextInt(5);
	...
	public Consumetable addOrder(final String userId, final Map<String, Integer> products, 
		...
		for (final Iterator<String> i = products.keySet().iterator(); i.hasNext(); i.remove()) {
			...
			// 微信端：withold的键为商品id，而不是期id！
			// pzp - 2015-09-25
			//withold.put(pid, buyingCount);
			withold.put(product.getProductId()+"", buyingCount);
