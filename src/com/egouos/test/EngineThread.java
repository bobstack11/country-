  ...
  extends Thread
{
  ...
  public void run()
  {
    while (!this.engineContainer.isEmpty())
    {
      String engineUrl = this.engineContainer.getEngineUrl();
      if (engineUrl == null) {
        return;
      }
      try
      {
        Proxy proxyIp = this.proxyContainer.getProxy();
        System.err.println("启动线程：" + this.threadName + "代理IP：" + proxyIp.getIp() + "端口：" + proxyIp.getPort());
        ...
        this.httpClient.getParams().setParameter("compatibility", null);
        
        this.httpGet = new HttpGet(engineUrl);
        ...
        String html = (String)this.httpClient.execute(this.httpGet, responseHandler);
        ...
        for (Element link : links)
        {
          ...
          if (linkText.indexOf("1元夺宝 - 享受购物的乐趣") != -1)
          {
            ...
            HttpResponse entity123 = this.httpClient.execute(getMethod);
      ...
      {
        this.proxyContainer.reduceTask();
      }
    }
  }
}
