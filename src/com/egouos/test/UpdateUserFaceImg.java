...
import com.egouos.util.ReadFile;
import java.io.File;
...
public class UpdateUserFaceImg
{
    ...
    throws Exception
  {
    ...
    for (String string : strList) {
      try
      {
        this.user = ((User)userList.get(i));
        if (this.user.getFaceImg().equals("/Images/defaultUserFace.png"))
        {
          System.err.println(string.split("-")[0]);
          
          Long time = Long.valueOf(System.currentTimeMillis());
          String path = "/faceImages/" + this.user.getUserId() + "_" + time + string.split("-")[1];
          System.err.println(path);
          String localPath = "c:\\UserImg\\" + this.user.getUserId() + "_" + time + string.split("-")[1];
          WeiboImg.getImage(string.split("-")[0], localPath);
          System.err.println(localPath);
          this.user.setFaceImg(path);
